    ...
    BlockingQuestion? BlockingQuestion = null)
{
    ...
    public static string ResolveContractState(bool usedFallback, string? reason = null, string? providerState = null)
    {
        if (!usedFallback)
            return "validated";

        if (!string.IsNullOrWhiteSpace(providerState) &&
            !string.Equals(providerState, "completed", StringComparison.OrdinalIgnoreCase))
        {
