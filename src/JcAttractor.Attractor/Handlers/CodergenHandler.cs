...
public class CodergenHandler : INodeHandler
{
    ...
    public async Task<Outcome> ExecuteAsync(GraphNode node, PipelineContext context, Graph graph, string logsRoot, CancellationToken ct = default)
    {
        ...
        {
            usedFallback = true;
            var fallbackReason = string.IsNullOrWhiteSpace(contractError)
        ...
        var contractReason = string.IsNullOrWhiteSpace(contractError) ? stageStatus.FailureReason : contractError;
        var providerState = ResolveProviderState(telemetry);
        var contractState = StageStatusContract.ResolveContractState(usedFallback, contractReason);
